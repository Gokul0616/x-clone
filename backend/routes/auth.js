...
router.post('/register', async (req, res) => {
  try {
    ...
    const token = generateToken(user._id); // Use _id

    const userResponse = user.toObject();
    delete userResponse.password;
    userResponse.id = userResponse._id; // Map _id to id for compatibility
...
router.post('/logout', auth, async (req, res) => {
  try {
    await User.findByIdAndUpdate(req.user._id, { // Use _id
      isOnline: false,
      lastActive: new Date()
    });
    res.status(200).json({
