...
class ComposeTweetScreen extends StatefulWidget {
  ...
  const ComposeTweetScreen({super.key, this.replyToTweet, this.quoteTweet});
  ...
  Future<void> _postTweet() async {
    ...
    if (success && mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            widget.replyToTweet != null ? 'Reply posted!' : 'Tweet posted!',
          ),
          backgroundColor: AppColors.successColor,
        ),
      );
  ...
  Future<void> _pickImage() async {
    if (_selectedImages.length >= AppConstants.maxImagesPerTweet) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'You can only add up to ${AppConstants.maxImagesPerTweet} images',
          ),
          backgroundColor: AppColors.warningColor,
        ),
      );
      ...
      child: Row(
        children: [
          CircleAvatar(
            ...
            child: widget.replyToTweet!.user?.profileImageUrl == null
                ? Text(
                    widget.replyToTweet!.user?.displayName
                            .substring(0, 1)
                            .toUpperCase() ??
                        'U',
                    style: const TextStyle(fontSize: 12),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              CircleAvatar(
                radius: 10,
                backgroundImage:
                    widget.quoteTweet!.user?.profileImageUrl != null
                    ? NetworkImage(widget.quoteTweet!.user!.profileImageUrl!)
                    : null,
                child: widget.quoteTweet!.user?.profileImageUrl == null
                    ? Text(
                        widget.quoteTweet!.user?.displayName
                                .substring(0, 1)
                                .toUpperCase() ??
                            'U',
                        style: const TextStyle(fontSize: 10),
          ...
          const SizedBox(height: 4),
          Text(widget.quoteTweet!.content, style: theme.textTheme.bodyMedium),
          // Display quoted tweet images
          if (widget.quoteTweet!.imageUrls != null &&
              widget.quoteTweet!.imageUrls!.isNotEmpty)
            Container(
              margin: const EdgeInsets.only(top: 8),
              height: 100,
              child: ListView.builder(
                scrollDirection: Axis.horizontal,
                itemCount: widget.quoteTweet!.imageUrls!.length,
                itemBuilder: (context, index) {
                  return Container(
                    margin: const EdgeInsets.only(right: 8),
                    width: 100,
                    decoration: BoxDecoration(
                      ...
                      image: DecorationImage(
                        image: NetworkImage(
                          widget.quoteTweet!.imageUrls![index],
                        ),
                        fit: BoxFit.cover,
